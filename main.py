...
def fix_latex_fractions(text: str) -> str:
    ...
    # JSON returned by vision models sometimes contains a single standalone
    # backslash between matrix rows. Preserve LaTeX commands, but normalize
    # those row separators to the double backslash required by KaTeX.
    def repair_matrix_rows(match):
        environment = match.group(1)
        body = re.sub(r"(?<!\\)\\(?!\\|[A-Za-z])", r"\\\\", match.group(2))
...
def safe_response_text(value: Any) -> str:
...
def clean_explanation(text):
    ...
    return fix_latex_fractions(safe_response_text(text))


def get_step_field(step: dict[str, Any], *keys: str) -> str:
...
def normalize_solution(
    ...
    answer = fix_latex_fractions(
        safe_response_text(raw_data.get("answer", ""))
    )
    ...
    if isinstance(raw_steps, list):
        ...
        for step in raw_steps:
            ...
            if isinstance(step, dict):
                title = safe_response_text(
                    get_step_field(step, "title", "heading", "name")
                )
                ...
                steps.append(
                    SolutionStep(
                        title="",
                        explanation=clean_explanation(step),
                        expression=None,
                        result=None
                    )
                )
...
def solve_image_with_ai(
    ...
    try:
        ...
        IMAGE QUESTION RULE (MANDATORY):
        - The JSON `question` field must contain ONLY the question visibly
          written in the uploaded image.
        - Never copy typed user text into `question`.
        - Typed user text is optional solving context, not part of the image.
        - If no readable question is visible in the image, return
          "Unable to read image clearly" in `question`.

        If you are given both typed text and an image, use the typed text as the instruction for solving the image.
        Solve the math problem shown in the image accordingly.
       ...
       ====================================
        ...
        Every mathematical expression MUST be valid KaTeX.

        General:

        - Never use plain text for mathematics.
        - Never use ASCII matrices.
        - Never use markdown code blocks.
        - Never use HTML.
        - Never invent LaTeX commands.
       ...
       ====================================
        ...
        Always render matrices using bmatrix.
       ...
       """
        if instruction.strip():
            prompt += (
                "\n\nUSER INSTRUCTION (NOT THE IMAGE QUESTION):\n"
                f"{instruction.strip()}\n"
                "Use this only to decide how to solve the uploaded image. "
                "Do not include it in the JSON `question` field."
            )
    except Exception as e:
    ...
    # If answer is missing, use the last step result
    if not raw_data.get("answer"):
        for step in reversed(raw_data.get("steps", [])):
            if step.get("result"):
                raw_data["answer"] = step["result"]
                break

    # Always use the question extracted from the image.
    # Never replace it with the typed instruction.
    image_question = "Image Question"

    if isinstance(raw_data, dict):
        image_question = clean_ocr_text(
            safe_response_text(raw_data.get("question", ""))
        )
...
def is_typed_math_problem(text: str) -> bool:
    """Whether composer text is a separate math problem, not an image instruction."""
    return bool(re.search(
        r"[=+*/^]|\\\\[A-Za-z]+|[²³⁴⁵⁶⁷⁸⁹√∫∑∞]|\d\s*[A-Za-z]|[A-Za-z]\s*\d",
        text.strip(),
    ))
...
async def chat_with_image(
    ...
    try:
        ...
        # Solve Text Question
        # -------------------------
        typed_problem = question.strip() and is_typed_math_problem(question)
        if question.strip() and (not files or typed_problem):
            text_result = solve_with_ai(question)

            problems.append({
                "source": "text",
                "question": text_result.question,
                "answer": text_result.answer,
                "steps": [step.model_dump() for step in text_result.steps],
                "suggestions": text_result.suggestions
            })

        # -------------------------
        # Solve Image Questions
        # -------------------------
        if files:
            # A typed equation is solved as its own Text Problem, so it must
            # not be passed to the vision model as an image instruction.
            image_instruction = "" if typed_problem else question
            ...
            for file in files:
                ...
                problems.append({
                    "source": "image",
                    "question": clean_ocr_text(image_result.question),
                    "answer": image_result.answer,
                    "steps": [step.model_dump() for step in image_result.steps],
                    "suggestions": image_result.suggestions
                })
...
def clean_ocr_text(text: str) -> str:
    ...
    # Protect LaTeX expressions and commands
    text = re.sub(
        r"\$\$.*?\$\$|\$.*?\$|\\\(.+?\\\)|\\\[.+?\\\]|\\begin\{.*?\}.*?\\end\{.*?\}|\\[A-Za-z]+",
        protect,
        text,
    ...
    phrase_fixes = [
        ...
        (r"find([A-Z])", r"find \1"),
        (r"Writethe", "Write the"),
        (r"Solvethe", "Solve the"),
        (r"Evaluatethe", "Evaluate the"),
        (r"Calculatethe", "Calculate the"),
        (r"Simplifythe", "Simplify the"),
        (r"Expressthe", "Express the"),
        (r"Whatis", "What is"),
        (r"Findthe", "Find the"),
        (r"Equationof", "Equation of"),
        (r"inthe", "in the"),
        (r"slopeintercept", "slope-intercept"),
        (r"\s*-\s*", " - "),
    ]
